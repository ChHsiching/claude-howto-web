...
基于 VitePress 的静态站点，以 4 种语言忠实呈现 [luongnv89/claude-howto](https://github.com/luongnv89/claude-howto) 的内容。

## 特性

- **多语言**: English、Tiếng Việt、中文、Українська — 使用 VitePress i18n
- **动态侧边栏**: 根据上游内容结构自动生成
- **自动同步**: 通过 git submodule 从上游拉取内容
- **CI/CD**: GitHub Actions 定时构建部署，也支持手动触发
...
- **定时触发**: 每日 03:17 UTC 执行
- **手动触发**: 在 Actions 标签页通过 `workflow_dispatch` 触发
- **智能构建**: 仅在上游有新提交时构建（无变更则跳过）
- **部署**: 构建并部署到 GitHub Pages
